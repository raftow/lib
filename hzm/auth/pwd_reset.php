...
$module_auth_dir_name = $module_dir_name = $file_dir_name;
require_once("$file_dir_name/../../../lib/afw/core/afw_autoloader.php");
$uri_module = UfwUrlManager::currentURIModule();

require_once("$module_auth_dir_name/../../../$uri_module/ini.php");
...
if (AfwSession::userIsAuthenticated()) {
        ...
        list($idn_correct, $idn_type) = AfwFormatHelper::getIdnTypeId($idn);

        if (!$idn_correct) {
                $result_message = AfwLanguageHelper::tt("FORMAT-SA-IDN", $lang);
        } elseif (!AfwFormatHelper::isCorrectMobileNum($mobile)) {
...
include("$file_dir_name/../web/hzm_header.php");
if ($desc_site) {
                ...
                <div class="modal_login modal-content">
                        ...
                        ?>
                                <div class="quote">
                                        <div class="quoteinn">
                                                <p>
                                                        <font color='red'><?= $result_message ?></font>
                                                </p>
                                        </div>
                                </div>
                        <?
                        ...
                        ?>
                        <div class="modal-body">
                                <h1><?= $user_prefix ?>قم بادخال البيانات التالية</h1>
                                <form id="formlogin0" name="formlogin0" method="post" action="<?php echo $action_page ?>" onSubmit="return checkForm();" dir="ltr" enctype="multipart/form-data">
                                        <?php
