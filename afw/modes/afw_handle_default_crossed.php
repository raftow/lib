...
for ($i = 0; $i < $nb_objs; $i++) {
        ...
        if(!$is_load)
        {
                if($obj->isChanged())
                {
                        ...
                        if(!$inserted)
                        {
                            if((AfwSession::config('MODE_DEVELOPMENT', false)) and ($objme) and ($objme->isSuperAdmin())) throw new AfwRuntimeEx ception($obj->sql_info ." insert failed : ".$obj->tech_notes, array("FIELDS_UPDATED"=>true));
                        }
