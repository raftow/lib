...
foreach($class_db_structure as $nom_col => $desc)
{
        ...
        if(($desc["STEP"] == $obj->currentStep) or (!$obj->editByStep))
        {
            ...
            $i_can_see_attribute = ($obj->keyIsToDisplayForUser($nom_col_to_see, $objme, "DISPLAY") and 
                                    $obj->dataAttributeCanBeDisplayedForUser($nom_col_to_see, $objme, "DISPLAY", $desc)) ;
        ...
        {
            $mode_field_read_only = true;
            $mode_field_read_only_log = "$nom_col is not in step ".$desc["STEP"];
...
<?
        ...
        if($finish_label and ($obj->canFinishOnCurrentStep()  or 
