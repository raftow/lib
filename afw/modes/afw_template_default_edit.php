...
foreach ($class_db_structure as $nom_col => $desc) {
        ...
        if($nom_col=="desiresLimitWarningDiv")
        {
                ...
                $y = is_array($desc["STEPS"]);
                $z = var_export($desc["STEPS"], true);
                $w = $obj->currentStep;
        ...
        if ($mode_field_edit and $mode_field_read_only and (!$i_can_see_attribute)) {
                $mode_field_read_only = false;
                $mode_field_edit = false;
                $mode_field_edit_log .= ": $nom_col i can not see it so cancel mode edit/readonly";
        }
        ...
        if ($nom_colIsApplicable) {
                ...
                if ($mode_field_edit) {
                        if (!$mode_field_read_only) {
                                ...
                                $data[$nom_col] = AfwShowMotor::prepareShowInfoForColumn($obj, $nom_col, $desc, $lang, $obj_errors, $step_show_error, $i_can_see_attribute, $mode_field_read_only_log);
