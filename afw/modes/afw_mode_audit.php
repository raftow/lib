...
foreach($attribute_arr as $attribute)
{
        ...
        if($show!="merged")
        {
                ...
                CmsMainPage::addOutput($html);
                CmsMainPage::addOutput("</div>");
                CmsMainPage::addOutput("<br><hr>");
        }
}
...
foreach($attribute_arr as $attribute)
{
    ...
    foreach($audit_data[$attribute] as $audit_key => $audit_row)
    {
            ...
            $step++;
            /*
            CmsMainPage::addOutput( "<br>step $step : (attribute=$attribute, audit_key=$audit_key) audit_data_all = ".var_export($audit_data["all"],true); 
...
{
        CmsMainPage::addOutput("<div id='audit-$cl-$id-all'>");
