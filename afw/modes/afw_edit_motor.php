...
class AfwEditMotor {
    ...
    <?php
        ...
        <?php
            ...
            case 'MENUM':
                ...
                if($obj) $liste_rep = AfwLoadHelper::getEnumTable($desc["ANSWER"], $obj->getTableName(), $fcol_name, $obj);
                else $liste_rep = [1=>"no-obj-no-multi-answer-enum"];
            ...
            case 'ENUM':
                ...
                if ($desc["ANSWER"] == "INSTANCE_FUNCTION") {
                    if($obj) $liste_rep = AfwStructureHelper::getEnumAnswerList($obj, $orig_col_name);
                    else $liste_rep = [1=>"no-obj-no-instance-answer-enum"];
                    ...
                    else $liste_rep = [1=>"no-obj-no-answer-enum"];
                }
                //if(!$liste_rep) 
            ...
            case 'TEXT':
                ...
                if ((isset($desc["SIZE"])) && (($desc["SIZE"] == "AREA") or ($desc["SIZE"] == "AEREA"))) {
                    ...
                    if($obj) $fromList = $obj->$fromListMethod();
                    else $fromList = [1=>"no-obj-no-text-from-list"];
                    //echo "val=$val<br>";
                    self::select(
                        $fromList,
                        array(trim($val)),
    ...
    public static function clock($col_name, $input_name, $valaff, $minimum, $maximum, $onchange, $input_style = "", $precision = 10, $required = false, $separator = ':', $duration = false, $durationNegative = true)
    {
        if ($required) {
            $required = 'true';
    ...
    public static function mobiselector($list_id_val, $selected = array(), $info = array())
    {
    ...
    public static function select($list_id_val, $selected = array(), $info = array(), $sort_order = "", $null_val = true, $langue = "")
    {
        $lang = AfwLanguageHelper::getGlobalLanguage();
