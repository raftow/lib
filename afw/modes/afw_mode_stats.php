...
if($config_stats_super_header)
{
      ...
      foreach($config_stats_super_header as $config_stats_super_header_col)
      {
           ...
           if($class_xqe_col=="x") $class_xqe_col = "z";
...
foreach($stats_data_arr as $stats_curr_row => $stats_data_item)
{
    ...
    foreach($stat_trad as $col => $info)
    {
	     ...
	     $out_scr .= "      <td $class_xqe_prop align='$aligntd'>$val_stat</td>";
...
if($global_footer_sum)
{
      $out_scr .="
        ...
        foreach($stat_trad as $col => $info)
        {
                   ...
                   if($footer_sum_title_arr[$col]) $footer_total_val = $footer_sum_title_arr[$col];
                   else $footer_total_val = $footer_total_arr[$col];     
...
if($show_pie)
{
    if($show_pie == "FOOTER")
    {
        foreach($stat_trad as $col => $info)
        {
            if(!$footer_sum_title_arr[$col]) $data_pie[$info] = $footer_total_arr[$col];
        }
    }
