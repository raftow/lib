...
class UfwReplacement {
    ...
    public static function trans_replace($string, $module_code, $lang)    
    {
        ...
        if(!$company)
        {
            throw new AfwRuntimeException("company code required to do UfwReplacement::trans_replace (see main_company param in system config file)");
        }

        if(!$module_code)
        {
            throw new AfwRuntimeException("module code required to do UfwReplacement::trans_replace");
        }
