...
{
        public static function hidden_input($col_name, $desc = null, $val = '', &$obj)
        {
                // if (!$desc and $obj) $desc = AfwStructureHelper::getStructureOf($obj, $col_name);
        ...
        public static function text_input($col_name, $desc, $val, &$obj, $separator, $data_loaded = false, $force_css = '', $qedit_orderindex = 0, $data_length_class_default_for_fk = 'inputmoyen', $debugg = false)
        {
                ...
                switch ($desc['TYPE']) {
                                        ...
                                        if (true) {
                                                ...
                                                $html .= "
                                                        ...
                                                        \$(\"#$col_name_atc\").autocomplete({
                                                                source: \"../lib/api/autocomplete.php?cl=$nom_class_fk&currmod=$nom_module_fk&clp=$clp&idp=$idp&modp=$modp&attp=$attp&depend=$depend \",
                                                                minLength: 0,
                        ...
                        case 'MFK':
                                ...
                                if ($desc['SEL_OPTIONS'])
                                        $infos_arr = array_merge($infos_arr, $desc['SEL_OPTIONS']);

                                if ($desc['SEL_CSS_CLASS'])
                                        $infos_arr['class'] = $desc['SEL_CSS_CLASS'];
                                ...
                                break;
                        case 'MENUM':
                                $liste_rep = AfwStructureHelper::getEnumAnswerList($obj, $col_name, $desc['ANSWER']);
                        ...
                        case 'ENUM':
                                ...
                                if ($desc['FORMAT-INPUT']) {
                                        if ($desc['FORMAT-INPUT'] == 'hzmtoggle') {
                                                ...
                                                if (!$display_val)
                                                        $display_val = "...<!-- $val from " . var_export($liste_rep, true) . ' -->';
                                                ...
                                                if (!$val)
                                                        $val = 0;
                                                if ($desc['EMPTY_IS_ALL'])
                                                        $info['empty_item'] = $placeholder;
                                                else
                                                        $info['empty_item'] = '';

                                                // to be shown it is not in list add it (and after see what's the problem)
                                                if (($val) and (!$liste_rep[$val]))
                                                        $liste_rep[$val] = $val;
                                ...
                                if ((isset($desc['SIZE'])) && (($desc['SIZE'] == 'AREA') or ($desc['SIZE'] == 'AEREA'))) {
                                        ...
                                        if (!$desc['PLACEHOLDER-NO-CHANGE']) {
                                                if (($desc['MIN-SIZE']) and ($desc['MAXLENGTH'])) {
                                                        if ($placeholder)
                                                                $placeholder .= ' : ';
                                                        $nb_words_between = AFWObject::gtr('Number of words between [min] and [max] words', $lang, array('[min]' => $desc['MIN-SIZE'], '[max]' => $desc['MAXLENGTH']));
        ...
        public static function drop_down($list_id_val, $selected = array(), $info = array(), $sort_order = '', $null_val = true, $langue = '', $data_images = null)
        {
                ...
                if ($null_val) {
                        if ($info['required']) {
                                if (!$info['default_value'])
                                        $option_empty_html = '<option></option>';
                        } else {
                ...
                foreach ($list_id_val as $id => $val) {
                        ...
                        if ($val instanceof AFWObject)
                                $val_option = $val->getDropDownDisplay($langue);
                        else
                                $val_option = $val;
                ...
                if ($info['numberDisplayed'])
                        $multi_select_options .= " numberDisplayed: '" . $info['numberDisplayed'] . "',";
                if ($info['buttonWidth'])
                        $multi_select_options .= " buttonWidth: '" . $info['buttonWidth'] . "',";
                if ($info['dropRight'])
                        $multi_select_options .= ' dropRight: true,';
                if ($info['inheritClass'])
                        $multi_select_options .= ' inheritClass: true,';
                if ($info['enableFiltering'])
                        $multi_select_options .= ' enableFiltering: true,';
                if ($info['filterBehavior'])
                        $multi_select_options .= " filterBehavior: '" . $info['filterBehavior'] . "',";
                if ($info['filterPlaceholder'])
                        $multi_select_options .= " filterPlaceholder: '" . $info['filterPlaceholder'] . "',";
                if ($info['maxHeight'])
                        $multi_select_options .= ' maxHeight: ' . $info['maxHeight'] . ',';
                if ($info['includeSelectAllOption'])
                        $multi_select_options .= ' includeSelectAllOption: true,';
...
\$(\"#$id_input\").data(\"dd\");
        ...
        {
                if ($width > 0)
                        $props['style'] = "style='width:${width}px'";
                $props['class'] = $css;
                $props['name'] = $name;
                $props['id'] = $id;
        ...
        public static function inputErrorsInRequest($attribute, $data_error, $suffix = '_error')
        {
                ...
                if ($error_message)
                        return "<div id=\"attr_error_$attribute\" class=\"$attribute front error\" for=\"$attribute\">$error_message</div>";
