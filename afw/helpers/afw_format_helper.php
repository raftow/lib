...
{
    public static $COLORS = ['blue', 'cyan', 'purple', 'green', 'yellow', 'orange', 'red', 'gray']; // , 'white', 'black'
    ...
    final public static function isFormatted($desc)
    {
        if ($desc['FORMAT']) {
            return true;
        }
    ...
    final public static function decodeSimulatedFieldValue(&$object, $attribute, $field_value)
    {
        ...
        switch ($typattr) {
            ...
            case 'FK':
                ...
                if (!$attribute_value) {
                    ...
                    $return = AfwLoadHelper::decodeLookupValue($ans_module, $ans_table, $attribute_value, $items_separator, $items_empty_message, $pk, $lang, $small_lookup);
            ...
            case 'MFK':
                ...
                $small_lookup = $structure['SMALL-LOOKUP'];
                $return = AfwLoadHelper::lookupDecodeValues($ans_module, $ans_table, $attribute_value, $items_separator, $items_empty_message, $pk, $lang, $small_lookup);
                // $return = "$return = AfwLoadHelper::lookupDecodeValues($ans_module, $ans_table, $attribute_value, $items_separator, $items_empty_message,$pk, $lang, $small_lookup)";
    ...
    public static function formatPhone($phone_num, $region_id = 1, $country = 'SA')
    {
        ...
        if (strlen($phone_num) == 7) {
            $phone_num = $prefix_reg . $phone_num;
        }
    ...
    public static function formatMobile($mobile_num, $country = 'SA')
    {
        ...
        if ($missed > count($left_complete[$country])) {
            $missed = 0;
        }
        ...
        for ($k = 0; $k < $missed; $k++) {
            $mobile_num = $left_complete[$country][$k] . $mobile_num;
        }
    ...
    public static function getIdnTypeId($id_number, $authorize_other_sa_idns = false, $authorize_nid = false)
    {
        try {
            ...
            return [$idn_correct, $type];
        } catch (Exception $e) {
            return [false, $e->getMessage()];
        } catch (Error $e) {
            return [false, $e->__toString()];
        }
    }
    ...
    final public static function formatITEMS(&$object, $attribute, $structure, $table_name, $call_method, $max_items, $eager = false)
    {
        ...
        if (!$return) {
            ...
            if ($ansTab) {
                ...
                if ($eager) {
                    $return = $objectITEM->loadManyEager($limit_loadMany, $structure['ORDER_BY']);
                } else {
    ...
    public static function pbm_result($err, $info, $warn = null, $sep = "<br>\n", $tech = '', $result_arr = [], $limitSize = null)
    {
        ...
        if (is_array($info)) {
            $info = implode($sep, $info);
        }

        if (is_array($warn)) {
            $warn = implode($sep, $warn);
        }
    ...
    public static function getElementFromArrayByPath($array, $path, $separator = '.')
    {
        ...
        // die("getElementFromArrayByPath will return : ".var_export($temp, true));

        // Return the final value found at the specified path
        return [$temp, $log_arr];
    }
