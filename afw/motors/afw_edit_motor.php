<?php
    class AfwEditMotor
    {
        ...
        */
            ...
            if ((! $placeholder) or ($placeholder == $placeholder_standard_code)) {
                if ($obj) {
                    if (($desc["MANDATORY"]) and ($desc["TYPE"] != "TEXT")) {
                        ...
                        if ($instruction == $instruction_code) {
                            $instruction = $obj->translateOperator("INSTR-STD", $lang);
                        }
            ...
            if ($data_loaded) {
                $data_loaded_class = "data_loaded";
            } else {
                $data_loaded_class = "data_notloaded";
            }
            ...
            */
            ...
            if (true) {
                ...
                // $onchange = str_replace("§row§", $o bj->qeditNum, $onchange);
                // $onchange = str_replace("§rowcount§", $qeditCount, $onchange);

                if ($desc["FOOTER_SUM"]) {
                    $onchange .= "qedit_col_total('$qeditNomCol',$qeditCount); ";
                }

                $after_change = $desc["AFTER-CHANGE"];
                // $after_change = str_replace("§row§", $o bj->qeditNum, $after_change);
                // $after_change = str_replace("§rowcount§", $qeditCount, $after_change);

                $onchange .= $after_change;
                if ($mode_qedit) {
                    $onchange .= "iHaveBeenChanged('$col_name'); ";
                } else {
    ...
    <?php
                ...
                switch ($desc["TYPE"]) {
                    case 'PK':
                        if ($val <= 0) {
                            $descHid        = [];
                            $type_input_ret = self::hidden_input($col_name, $descHid, $val, $obj);
                            break;
                        } else {
                            $type_input_ret = "text";
                        ?>
        ...
        <?php
                        ...
                        case 'FK':
                            ...
                            if ($force_css) {
                                $data_length_class = " " . $force_css;
                            } else {
                                $data_length_class = " " . $data_length_class_default_for_fk;
                            }
                        ...
                        case 'ENUM':
                            ...
                            if ((! $desc["ENUM_ALPHA"]) and ((! $val) or (! intval($val)))) {
                                $val = 0;
                            }
                            ...
                            if ($desc["ANSWER"] == "INSTANCE_FUNCTION") {
                                if ($obj) {
                                    ...
                                    $liste_rep = [1 => "no-obj-no-instance-answer-enum"];
                                }
                        ...
                        case 'AMNT':
                            ...
                            if ($fromListMethod) {
                                if ($obj) {
                                    ...
                                    $fromList = [1 => "no-obj-no-amount-from-list"];
                                }
                                ...
                                if ($force_css) {
                                    ...
                                    $data_length_class = " inputcourt";
                                }

                                $type_input_ret = "text";
                                if ($obj and $desc["JS-COMPUTED"]) {
                                    if ($obj->class_of_input_computed_readonly) {
                                        $class_of_input = $obj->class_of_input_computed_readonly;
                                    }

                                    if ($obj->class_js_computed) {
                                        $class_js_computed = $obj->class_js_computed;
                                    } else {
                                        $class_js_computed = "js_computed";
                                    }
                        ...
                        case 'TIME':
                            if ($desc["FORMAT"] == "CLOCK") {
                                ...
                                } elseif ($obj and ($desc["FORMAT"] == 'OBJECT')) {
                                    $helpMethod  = $desc["ANSWER_METHOD"];
                            ...
                            if ((isset($desc["SIZE"])) && (($desc["SIZE"] == "AREA") or ($desc["SIZE"] == "AEREA"))) {
                                ...
                                if (! $desc["PLACEHOLDER-NO-CHANGE"]) {
                                    if (($desc["MIN-SIZE"]) and ($desc["MAXLENGTH"])) {
                                        ...
                                        $nb_words_between = AFWObject::gtr("Number of words between [min] and [max] words", $lang, ["[min]" => $desc["MIN-SIZE"], "[max]" => $desc["MAXLENGTH"]]);
                                        $placeholder .= $nb_words_between;
                                    } elseif ($desc["MIN-SIZE"]) {
                                        if ($placeholder) {
                                            $placeholder .= " : ";
                                        }

                                        $nb_words_min = AFWObject::gtr("Minimum number of words [min] words", $lang, array("[min]" => $desc["MIN-SIZE"]));
                                        $placeholder .= $nb_words_min;
                                    } elseif ($desc["MAXLENGTH"]) {
                                        if ($placeholder) {
                                            $placeholder .= " : ";
                                        }

                                        $nb_words_max = AFWObject::gtr("Maximum number of words [max] words", $lang, array("[max]" => $desc["MAXLENGTH"]));
                                ...
                                if ($obj) {
                                    ...
                                    $fromList = [1 => "no-obj-no-text-from-list"];
                                }
                                ...
                                if ($desc["INPUT-FORMATTING"] == "value-1-cote") {
                                    $val_sentence = "value='$val'";
                                } else {
                                    $val_sentence = "value=\"$val\"";
                                }

                                if (($force_css) and (! $desc["WIDTH-FROM-SIZE"])) {
                                    $data_length_class = " " . $force_css;
                                } else if (isset($desc["SIZE"]) && $desc["SIZE"] <= 16) {
                                    $data_length_class = " inputcourt";
                                } else if (isset($desc["SIZE"]) && $desc["SIZE"] <= 41) {
                                    $data_length_class = " inputmoyen";
                                } else if (isset($desc["SIZE"]) && $desc["SIZE"] <= 84) {
                                    $data_length_class = " inputlong";
                                } else if (isset($desc["SIZE"]) && $desc["SIZE"] < 255) {
                                    $data_length_class = " inputtreslong";
                                } else {
                                    $data_length_class = " inputultralong";
                                }
                        ...
                        case 'YN':
                            ...
                            if ($obj) {
                                ...
                                if (! $remove_options_arr["N"]) {
                                    $this_no_label = $obj->showYNValueForAttribute("NO", $col_name, $lang);
                                }

                                if (! $remove_options_arr["W"]) {
                                    $this_dkn_label = $obj->showYNValueForAttribute("EUH", $col_name, $lang);
                                }

                            } else {
                                list($this_yes_label, $this_no_label, $this_dkn_label) = AfwLanguageHelper::translateYesNo("decodeme", $lang);

                            }

                            if (! $remove_options_arr["Y"]) {
                                $answer_list["Y"] = $this_yes_label;
                            }

                            if (! $remove_options_arr["W"]) {
                                $answer_list["W"] = $this_dkn_label;
                            }

                            if (! $remove_options_arr["N"]) {
                                $answer_list["N"] = $this_no_label;
                            }
                        ...
                        case 'GDATE':
                            ...
                            if (! $val) {
                                if (trim(strtolower($desc["DEFAULT"])) == "today") {
                                    $val = date("Y-m-d");
                                }
    ...
    <?php
            ...
            public static function select($list_id_val, $selected = [], $info = [], $sort_order = "", $null_val = true, $langue = "")
            {
                $lang = AfwLanguageHelper::getGlobalLanguage();
                if (! $langue) {
                    $langue = $lang;
                ...
                if ($sort_order) {
                    $list_val = [];
                    foreach ($list_id_val as $id => $val) {
                        if ($val instanceof AFWObject) {
                            $list_val[$id] = $val->getDropDownDisplay($langue);
                        } else {
                            $list_val[$id] = $val;
                        }
        ...
        <script>
            <?php
                        ...
                        if ($info["loadmyprops"]) {
                            echo "// loadmyprops func-tion for attribute : " . $info["name"] . "\n";
                            echo $info["loadmyprops"] . "\n\n";
                        }

                        if ($info["reloadfn"]) {
                            echo "// reload func-tion for attribute : " . $info["name"] . "\n";
                            echo $info["reloadfn"] . "\n\n";
                        }
        ...
        <select class="<?php echo $info["class"] ?>" name="<?php echo $info["name"] ?>" id="<?php echo $info["id"] ?>" tabindex="<?php echo $info["tabindex"] ?>" onchange="<?php echo $info["onchange"] ?>"<?php echo $multi ?> size=<?php echo $size ?> <?php echo $info["style"] ?> <?php if ($info["disable"] or $info["disabled"]) {
                 echo "disabled";
         }
         ?> <?php if ($info["required"]) {
                 echo "required";
    ...
    <?php
            ...
            public static function attributeEditDiv($obj, $col, $desc, $fgroup, $lang, $openedInGroupDiv, $info = null, $colErrors = [], $step_show_error = false)
            {
                ...
                if (! $css_custom) {
                    if ($desc["CATEGORY"] == "ITEMS") {
                        $css_custom = "width_pct_100";
                    } elseif ($desc["TYPE"] == "MFK") {
                        $css_custom = "width_pct_100";
                    } elseif ($desc["SIZE"] == "AREA") {
                        $css_custom = "width_pct_100";
                    } elseif ($desc["TYPE"] == "DATE") {
                        $css_custom = "width_pct_50";
                    } elseif ($desc["TYPE"] == "GDAT") {
                        $css_custom = "width_pct_50";
                    } elseif ($desc["SIZE"] < 33) {
                        $css_custom = "width_pct_25";
                    } elseif ($desc["SIZE"] < 43) {
                        $css_custom = "width_pct_33";
                    } elseif ($desc["SIZE"] < 67) {
                        $css_custom = "width_pct_50";
                    } elseif ($desc["SIZE"] < 85) {
                        $css_custom = "width_pct_66";
                    } elseif ($desc["SIZE"] < 101) {
                        $css_custom = "width_pct_75";
                    } else {
                        $css_custom = "";
                    }
                ...
                if (! $desc["NO-LABEL"]) {
                    ...
                    if ($info["trad"]) {
                        ...
                        if ($info["warning"]) {
                            echo '<br><div class="ewarning">' . $info["warning"] . '</div>';
                        }
                        ...
                        if ($info["help"]) {
                            echo '<span class="hspan">' . $info["help"] . '</span>';
                        }
                ...
                if ($info["error"] and $desc["ERROR-SHOW"]) {
                    $htmlDiv .= "$br_if_needed<div id='attr_error_$col' class='error' for='$col'>" . $info["error"] . "</div>";
                }
                //
                ...
                if ($info["unit"] or $info["tooltip"] or $info["error"]) {
                    ...
                    if ($info["tooltip"] or $info["error"]) {
                        $css_input_width_pct -= 10;
                    }

                    if ($info["unit"]) {
                        $css_input_width_pct -= 20;
                    }
                    ...
                    if ($info["tooltip"] or $info["error"]) {
                        if ($info["error"] and (! $desc["ERROR-HIDE"])) {
                            ...
                            $htmlDiv .= "<div class=\"hzm_tooltip\"><img data-toggle=\"tooltip\" data-placement=\"left\" class=\"hzm_tt\" title=\"" . $info["tooltip"] . "\" src=\"../lib/images/information.png\" /></div>";
                        }

                    }

                    $htmlDiv .= $info["input"];
                    if ($info["unit"] and (! $info["no-hzm-unit"])) {
                        $htmlDiv .= "<div class=\"hzm_unit\">" . $info["unit"] . "</div>";
                    }
                ...
                if ($info["ehelp"]) {
                    $htmlDiv .= "$br_if_needed<div class='ehelp'>" . $info["ehelp"] . "</div>";
                }
                //
            ...
            public static function prepareEditInfoForColumn($obj, $nom_col, $desc = null, $lang = "ar", $colErrors = [], $step_show_error = false)
            {
                ...
                if (($desc['TYPE'] == 'PK') && empty($col_val)) {
                    $data_loaded = false;
                    self::type_input($nom_col, $desc, $id, $obj, $separator, $data_loaded);
                } else {
                    self::type_input($nom_col, $desc, $col_val, $obj, $separator, $data_loaded, "inputlong", 0, "inputlong");
                }
                ...
                if ($val_help != $col_help) {
                    $data_col["help"] = $val_help;
                }
                ...
                if ($data_col["unit_explain"]) {
                    $data_col["unit"] = "الوحدة = " . $data_col["unit"];
                }
            ...
            public static function calendar_translations($lang)
            {

                if ($lang == "ar") {
                    return 'monthNames: ["يناير","فبراير","مارس","أبريل","ماي","يونيو", "يوليو","أغسطس","سبتمير","أكتوبر","نوفمبر","ديسمبر"],
