...
class AfwDataMigrator extends AFWRoot {
    ...
    public static function migrateData($migration_config, $partition=0, $phase="", $case="", $lang="ar", $returnLog=false, $forced_print_full_debugg=true)
    {
        ...
        else UfwBatch::print_warning(implode("<br>\n",$log2_arr));
        ...
        if($destinationClass and $migration_config["startByReset"])
        {
            ...
            else UfwBatch::print_warning($res_log);
        }
        ...
        foreach($data as $irow => $row)
        {
            try
            {
                ...
                {
                    // UfwBatch::print_warning("going to exec migrateRecord(\$row, \$mapping, destination=$destination, destination_db=$destination_db, destination_table=$destination_table,...)");
                ...
                if($destinationClass)
                {
                    ...
                    if($mem_picture_counter==$updateProgressFrequency)
                    {
                        ...
                        if(!$disable_tab_instances_log)
                        {
                            ...
                            UfwBatch::print_warning($res_log);
                            unset($res_log);
                        }                    
        ...
        {
            UfwBatch::print_warning($result_log);
    ...
    public static function migrateRecord($row, $mapping, $destination, $destination_db, $destination_table, $lang, $destination_pk_cols, $destination_cols,  $last_update_value, $forced_print_full_debugg)
    {
        ...
        {
            UfwBatch::print_debugg("row=".var_export($row,true).", rowMapped=".var_export($rowMapped,true));             
        }
        $created = 0;
        $updated = 0;
        $skipped = 0;
    ...
    public static function mapAndTransformRow($row, $mapping, $destinationClass, $forcd_print_full_debugg=false, $advanced_log=false)
    {
        ...
        foreach($mapping as $colToMapX => $mappingCol)
        {
            ...
            if($transformation and $transformationClass)
            {
                ...
                {
                    UfwBatch::print_debugg("[$print_full_debugg or $forcd_print_full_debugg] colToMap=$colToMap : $old_value => $transformationClass::$transformation => $value"); 
                }
            ...
            if($transformation2 and $transformation2Class)
            {
                ...
                if($advanced_log and ($print_full_debugg or $forcd_print_full_debugg))
                {
                    UfwBatch::print_debugg("[$print_full_debugg or $forcd_print_full_debugg] colToMap=$colToMap : $old_value => $transformation2Class::$transformation2 => $value"); 
                }
                if($transformed) $no_transformation = false;
            }
            ...
            if($transformationDestination and $transformationDestinationClass)
            {
                ...
                if($advanced_log and ($print_full_debugg or $forcd_print_full_debugg))
                {
                    UfwBatch::print_debugg("[$print_full_debugg or $forcd_print_full_debugg] colToMap=$colToMap : $old_value => $transformationDestinationClass::$transformationDestination => $value"); 
                }
                if($transformed) $no_transformation = false;
            }
