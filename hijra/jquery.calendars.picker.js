...
function CalendarsPicker() {
	this._defaults = {
		...
		yearRange: 'any', // Range of years to show in drop-down: 'any' for direct text entry
	...
	this.regional[''] = { // Default regional settings
		...
		prevText: '&lt;السابق', // Text for the previous month command
		prevStatus: 'أظهر الشهر السابق', // Status text for the previous month command
		prevJumpText: '&lt;&lt;', // Text for the previous year command
		prevJumpStatus: 'السنه السابقة', // Status text for the previous year command
		nextText: 'التالي&gt;', // Text for the next month command
		nextStatus: 'أظهر الشهر التالي', // Status text for the next month command
		nextJumpText: '&gt;&gt;', // Text for the next year command
		nextJumpStatus: 'السنه التالية', // Status text for the next year command
		currentText: 'الحالي', // Text for the current month command
		currentStatus: 'الشهر الحالي', // Status text for the current month command
		todayText: 'اليوم', // Text for the today's month command
		todayStatus: 'شهر اليوم', // Status text for the today's month command
		clearText: 'إلغاء', // Text for the clear command
		clearStatus: 'إلغاء', // Status text for the clear command
		closeText: 'أغلق', // Text for the close command
		closeStatus: 'أغلق', // Status text for the close command
		yearStatus: 'غير السنه', // Status text for year selection
		monthStatus: 'غير الشهر', // Status text for month selection
		weekText: 'الأسبوع', // Text for week of the year column header
		weekStatus: 'أسبوع السنه', // Status text for week of the year column header
		dayStatus: 'إختر DD, d M, yyyy', // Status text for selectable days
		defaultStatus: 'إختر اليوم', // Status text shown by default
...
$.extend(CalendarsPicker.prototype, {
	...
	_popupClass: 'well well-sm calendars-popup', // Marker for popup division
	_triggerClass: 'calendars-trigger', // Marker for trigger element
	_disableClass: 'calendars-disable', // Marker for disabled element
	_monthYearClass: 'ng-pristine', // Marker for month/year inputs
	_curMonthClass: 'calendars-month-', // Marker for current month/year
	_anyYearClass: 'form-control ng-pristine ng-valid ng-touched', // Marker for year direct input
	_curDoWClass: 'calendars-dow calendars-dow-', // Marker for day of week
...
//		picker: '<div class="well well-sm ng-isolate-scope ng-valid ng-valid-date ng-valid-date-disabled">' +
//		'<div class="navbar-header">{link:prev}{link:today}{link:next}</div>{months}' +
//		'{popup:start}<div class="calendars-ctrl">{link:clear}{link:close}</div>{popup:end}' +
//		'<div class="calendars-clear-fix"></div></div>',
		
		picker: '<div>' +
					'<div id="invalidUmmalQuraDate" class="alert alert-danger" style="visiblity:hidden;display:none">'+
						'<p  >'+ 
						'خطأ في التاريخ'+	
						'</p>'+
					'</div>'+
					'<nav class="navbar navbar-default calendar">' +
			
							'<div> ' +
								'<ul class="nav  well well-sm ">' +
									'<li>{link:prev}</li> '+
									'<li>{link:today}</li>'+
									'<li>{link:next}</li>' +
								'</ul>' +
					...
					'</nav>'+
					'{months}{popup:start}' +
					'<div class="calendar_head well well-sm ng-isolate-scope">'+
...
//			class="btn btn-primary dropdown-toggle" dropdown-toggle
		monthRow: '<div class="dropdown-toggle btn-group">{months}</div>',
		// A single month: '{monthHeader:dateFormat}' to insert the month header -
		// dateFormat is optional and defaults to 'MM yyyy',
		// '{weekHeader}' to insert a week header, '{weeks}' to insert the month's weeks
		month: '<div class="calendars-month"><div class="form-horizontal">{monthHeader}</div>' +
				...
				if (!inst.div) {
					inst.div = $('<div></div>').addClass(this._popupClass).
						css({display: (hidden ? 'none' : 'static'), position: 'absolute',
							left: target.offset().left, zIndex:'99999',
	...
	_generateContent: function(target, inst) {
		...
		picker.find(inst.options.renderer.daySelector + ' a').hover(
			...
			find('input.' + self._monthYearClass).change(function() {
				try {
					document.getElementById("invalidUmmalQuraDate").style.visibility='hidden';
					document.getElementById("invalidUmmalQuraDate").style.display='none';
					...
					//alert(e);
					document.getElementById("invalidUmmalQuraDate").style.visibility='visible';
					document.getElementById("invalidUmmalQuraDate").style.display='block';
				}
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
		...
		var selector = ' <div class="form-group col-sm-5"><select class="form-control  ' + this._monthYearClass + 
		...
		if (yearRange == 'any') {
			selector = '<div class="form-group col-sm-6"><select style="visibility:hidden;display:none" class="' + this._monthYearClass + ' ' + this._anyYearClass +
				'" title="' + inst.options.yearStatus + '">' +
				'<option>' + year + '</option></select>' +
				'<input type="number" title="' + inst.options.yearStatus + '" class="form-control ' + this._monthYearClass + 
				 '" value="' + year + '"></div>';
		}
		else {
			...
			selector = '<div class="form-group col-sm-6"><select class="' + this._monthYearClass +' '+'form-control ng-pristine ng-valid ng-touched'+
				'" title="' + inst.options.yearStatus + '">';
