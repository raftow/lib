...
function hzm_wrap_mail($text, $indent = '') {
  // Convert CRLF into LF.
  $text = str_replace("\r", '', $text);
  // See if soft-wrapping is allowed.
  $clean_indent = _hzm_html_to_text_clean($indent);
  ...
  if (strpos($text, "\n") !== FALSE) {
    ...
    // Wrap this line.
    _hzm_wrap_mail_line($text, 0, array('soft' => $soft, 'length' => strlen($indent)));
  }
 ...
 */
function hzm_html_to_text($string, $allowed_tags = NULL) {
  ...
  foreach ($split as $value) {
    ...
    if ($tag) {
      ...
      switch ($tagname) {
        ...
        case '/h2':
          ...
          $output = _hzm_html_to_text_pad($output, ($tagname == '/h1') ? '=' : '-', ' ');
          array_pop($indent);
          $chunk = ''; // Ensure blank new-line.
          break;
        ...
        case 'hr':
          // Insert immediately.
          $output .= hzm_wrap_mail('', implode('', $indent)) . "\n";
          $output = _hzm_html_to_text_pad($output, '-');
          break;
    ...
    if (isset($chunk)) {
      ...
      $output .= hzm_wrap_mail($chunk, implode('', $indent)) . MAIL_LINE_ENDINGS;
 ...
 */
function _hzm_wrap_mail_line(&$line, $key, $values) {
 ...
 */
function _hzm_html_to_mail_urls($match = NULL, $reset = FALSE) {
 ...
 */
function _hzm_html_to_text_clean($indent) {
 ...
 */
function _hzm_html_to_text_pad($text, $pad, $prefix = '') {
