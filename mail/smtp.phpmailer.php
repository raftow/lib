...
class PHPMailer {
 ...
 /**
  ...
  public   $smtp           = NULL;   //  @todo:rafik  reput it private
  private   $to             = array();
  private   $cc             = array();
  private   $bcc            = array();
  ...
  public function __construct($exceptions = FALSE) {
    $this->logging = AfwSession::config('smtp_debugging', false);
    ...
    try {
      while ($index < count($hosts) && !$connection) {
        ...
        if (preg_match('/^(.+):([0-9]+)$/', $hosts[$index], $hostinfo)) {
          ...
          // die("this->Host = $this->Host case 1, hosts[$index]=".$hosts[$index]." => host = $host, port = $port");
        }
        else {
          $host = $hosts[$index];
          $port = $this->Port;
          // die("this->Host = $this->Host case 2, hosts[$index]=".$hosts[$index]." => host = $host, port = $port");
        }
        ...
        $myHost = ($ssl ? 'ssl://':'') . $host;
        $tout = $this->Timeout;
        if ($this->smtp->Connect($myHost, $port, $tout)) {
  ...
  */
  ...
  protected function SetError($msg) {
    ...
    if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
      ...
      {
        $msg .= '<p>strange error : ' . var_export($lasterror,true) . "</p>\n";
      }
    }
    $this->ErrorInfo = $msg;
  }
