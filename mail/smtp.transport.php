...
class SMTP {
  ...
  public $error;     // @todo:rafik  reput it private // error if any on the last call
  ...
  public function Connect($host, $port = 0, $tval = 30) {
    ...
    if (empty($this->smtp_conn)) {
      // die("fsockopen($host, $port, $errno,   $errstr,  $tval)");
  ...
  public function Authenticate($username, $password) {
    ...
    if ($code != 334) {
      $this->error =
        array("error" => "Username $username not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply, 4));
      if ($this->do_debug >= 1) {
        echo "SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF . '<br />';
    ...
    if ($code != 235) {
      $this->error =
        array("error" => "Password $username/$password not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply, 4));
      if ($this->do_debug >= 1) {
        echo "SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF . '<br />';
